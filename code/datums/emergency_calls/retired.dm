/datum/emergency_call/retired
	name = "Retired SFMC Veteran Squad"
	base_probability = 5
	alignement_factor = -1
...
/datum/emergency_call/retired/create_member(datum/mind/mind_to_assign)
	...
	if(!leader)
		...
		to_chat(H, "<p style='font-size:1.5em'><span class='notice'>You are the SFMC retired veteran expedition leader! Lead your fellow veterans to one last hurrah!</notice></p>")
		return

	var/datum/job/J = SSjob.GetJobType(/datum/job/retired)
	H.apply_assigned_role_to_spawn(J)
	to_chat(H, "<p style='font-size:1.5em'><span class='notice'>You are an augmented SFMC veteran, you may have had a few limbs replaced with synthetic versions, but at least you can walk! Follow the expedition leader and relive your glory days!</notice></p>")
