...
/datum/emergency_call/vsd/print_backstory(mob/living/carbon/human/H)
	...
	to_chat(H, "<B>You are equipped with Ballistic Armor to counter some of SFMC's weaponry. Jaeger's armor has a weakness, aim for the head and just below that chest plate. Jaeger has a weakness to 5.56.</b>")
	to_chat(H, "<B>A SFMC vessel has entered Syndicate and I.C.C. airspace. You're here to take down the ship. Goodluck.</b>")
	to_chat(H, "")
...
/datum/emergency_call/vsd/create_member(datum/mind/M)
	...
	if(!leader)
		...
		to_chat(H, "<p style='font-size:1.5em'><span class='notice'>You are the leader of the Grunwald-Svarog Werke 'Death Squad' group. Lead your men to victory, leave no trace. Hoorah!</notice></p>")
		return
	...
	H.apply_assigned_role_to_spawn(J)
	to_chat(H, "<p style='font-size:1.5em'><span class='notice'>You are a trained Grunwald-Svarog Werke operative. You are no disposable. You are trained in every way possible to counter SFMC's weaponries. We will not accept any casualties here. Good luck grunt, Hoorah!</notice></p>")
