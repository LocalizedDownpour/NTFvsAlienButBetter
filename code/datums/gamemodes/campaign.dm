...
/datum/game_mode/hvh/campaign/announce()
	...
	to_chat(world, "<b>The fringe world of Palmaria is undergoing significant upheaval, with large portions of the population threatening to secede from SFMC. With the population on the brink of civil war, \
	both SFMC and the Sons of Mars forces are looking to intervene.")
...
/datum/game_mode/hvh/campaign/intro_sequence()
	...
	for(var/mob/living/carbon/human/human AS in GLOB.alive_human_list)
		if(human.faction == factions[1])
			human.play_screen_text(HUD_ANNOUNCEMENT_FORMATTING(op_name_faction_one, "Fight to restore peace and order across the planet, and check the SOM threat.<br>" + "[GAME_YEAR]-[time2text(world.realtime, "MM-DD")] [stationTimestamp("hh:mm")]<br>" + "SFMC Rapid Reaction Battalion<br>" + "[human.job.title], [human]<br>", LEFT_ALIGN_TEXT), /atom/movable/screen/text/screen_text/picture/rapid_response)
...
/datum/game_mode/hvh/campaign/end_round_fluff()
	...
	switch(round_finished)
		...
		if(MODE_COMBAT_PATROL_MARINE_MINOR)
			announcement_body = "SFMC forces have routed the terrorist SOM forces across the planet, destroying their strongholds and returning possession of stolen property to their legitimate corporate owners. \
...
/datum/game_mode/hvh/campaign/record_death(datum/source, mob/living/carbon/player, override = FALSE)
	. = ..()
	if(!.)
		return
...
/datum/game_mode/hvh/campaign/auto_attempt_respawn(respawnee_ckey)
	. = ..()
	respawn_timers[respawnee_ckey] = null

/datum/game_mode/hvh/campaign/do_respawn(mob/new_player/ready_candidate, datum/job/job_datum)
	. = ..()
	if(!.)
		return
	if(current_mission.mission_state == MISSION_STATE_ACTIVE) //only free respawns between missions
		stat_list[job_datum.faction].active_attrition_points -= job_datum.job_cost

/datum/game_mode/hvh/campaign/respawn_checks(mob/new_player/ready_candidate, datum/job/job_datum)
	. = ..()
	if(!.)
		return
	if((current_mission.mission_state == MISSION_STATE_ACTIVE) && (stat_list[job_datum.faction].active_attrition_points < job_datum.job_cost))
...
/datum/game_mode/hvh/campaign/IsJobAvailable(mob/candidate, datum/job/job, faction)
	. = ..()
	if(!.)
		return
