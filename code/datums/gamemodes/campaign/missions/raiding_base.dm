/datum/campaign_mission/raiding_base
	...
	intro_message = list(
		...
		MISSION_HOSTILE_FACTION = "Stop SFMC forces from infiltrating the base. Prevent them from activating an orbital beacon at all costs!",
	)
	...
	hostile_faction_mission_brief = "Intelligence has picked up a SFMC plan to assault Raiding base Zulu. This base has been key to our sabotage and disruption efforts, significantly degrading SFMC supply lines. \
	Intel suggests that the SFMC are seeking to infiltrate the base to deploy a orbital beacon, in order to call down an orbital strike. \
	Prevent SFMC forces from entering the base, and destroy any orbital beacon they try to deploy."
	starting_faction_additional_rewards = "Remove negative effects on our logistics"
	hostile_faction_additional_rewards = "Allow us to continue degrading SFMC logistics"
	outro_message = list(
...
/datum/campaign_mission/raiding_base/som
	...
	intro_message = list(
		MISSION_STARTING_FACTION = "Infiltrate the SFMC base, then plant and defend a bluespace beacon until we can activate bluespace artillery!",
	...
	starting_faction_mission_brief = "We have finally been able to track down a hidden SFMC outpost which they have been using as a base of operations to raid our supply lines, wrecking havoc on our logistics. \
	Your unit has been tasked with ensuring the complete and utter destruction of this base and everything within it. \
	...
	outro_message = list(
		...
		MISSION_OUTCOME_MAJOR_LOSS = list(
			MISSION_STARTING_FACTION = "<u>Major loss</u><br> SFMC interceptors are inbound, all forces fallback, this operation is a loss.",
