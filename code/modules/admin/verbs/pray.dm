...
/client/verb/pray(msg as null)
	...
	if(liaison)
		to_chat(usr, "Your corporate overlords at SFMC have received your message.")
	else
...
/proc/tgmc_message(text, mob/sender)
	...
	for(var/client/C in GLOB.admins)
		if(check_other_rights(C, R_ADMIN, FALSE))
			to_chat(C, span_notice("<b><font color='purple'>To High Command:</font>[ADMIN_FULLMONTY(usr)] (<a href='byond://?src=[REF(C.holder)];[HrefToken(TRUE)];reply=[REF(sender)]'>REPLY</a>): [text]</b>"))
			SEND_SOUND(C, S)
