...
/obj/item/clothing/mask/gas/bdsm_mask
	...
	icon = 'modular_lewd_items/icons/obj/lewd_clothing/lewd_masks.dmi'
	icon_override = 'modular_lewd_items/icons/mob/lewd_clothing/lewd_masks.dmi'
	worn_icon_state = "mask_pink_off"
	...
	var/time_to_choke = 12
	var/time_to_choke_left
	var/time = 2
	var/tt
	...
	var/temp_check = TRUE
	var/speech_disabled = FALSE
	var/modifies_speech = TRUE

/obj/item/clothing/mask/gas/bdsm_mask/Initialize(mapload)
	. = ..()
	create_storage(storage_type = /datum/storage/bdsm_mask, canhold = list(/obj/item/reagent_containers/cup/lewd_filter))
...
/obj/item/clothing/mask/gas/bdsm_mask/proc/handle_speech(datum/source, list/speech_args)
	...
	conditional_pref_sound(loc, pick('modular_lewd_items/sounds/under_moan_f1.ogg',
						'modular_lewd_items/sounds/under_moan_f2.ogg',
						'modular_lewd_items/sounds/under_moan_f3.ogg',
						'modular_lewd_items/sounds/under_moan_f4.ogg'), 70, 1, -1)
...
/datum/action/item_action/toggle_breathcontrol/action_activate()
...
/datum/action/item_action/toggle_gag/action_activate()
	var/obj/item/clothing/mask/gas/bdsm_mask/mask = target
	if(istype(mask))
...
/datum/action/item_action/mask_inhale/action_activate()
	var/obj/item/clothing/mask/gas/bdsm_mask/mask = target
	if(!istype(mask))
		return FALSE
...
/obj/item/clothing/mask/gas/bdsm_mask/proc/toggle(mob/living/carbon/user)
	...
	conditional_pref_sound(user, mask_on ? 'sound/machines/click.ogg' : 'sound/machines/click.ogg', 40, TRUE)
	update_icon_state()
	update_mob_action_buttonss()
	update_icon()
	if(mask_on)
