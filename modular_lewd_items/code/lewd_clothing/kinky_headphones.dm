/obj/item/clothing/ears/earmuffs/kinky_headphones
	...
	worn_icon_state = "kinkphones_pink_off"
...
//to change model
/obj/item/clothing/ears/earmuffs/kinky_headphones/click_alt(mob/user)
	if(color_changed)
...
/obj/item/clothing/ears/earmuffs/kinky_headphones/proc/check_menu(mob/living/user)
	if(!istype(user))
		return FALSE
...
/obj/item/clothing/ears/earmuffs/kinky_headphones/equipped(mob/living/carbon/human/user, slot)
	. = ..()
...
/obj/item/clothing/ears/earmuffs/kinky_headphones/dropped(mob/living/carbon/human/user)
	. = ..()
	if(!(src == user.wear_ear))
		return
	REMOVE_TRAIT(user, TRAIT_DEAF, CLOTHING_TRAIT)
	to_chat(user, span_purple("You can finally hear the world around you once more."))

/obj/item/clothing/ears/earmuffs/kinky_headphones/Initialize(mapload)
	. = ..()
	update_icon_state()
	update_icon()
...
/obj/item/clothing/ears/earmuffs/kinky_headphones/update_icon_state()
	. = ..()
	icon_state = "[base_icon_state]_[current_kinkphones_color]_[kinky_headphones_on? "on" : "off"]"
	worn_icon_state = "[base_icon_state]_[current_kinkphones_color]_[kinky_headphones_on? "on" : "off"]"
	inhand_icon_state = "[base_icon_state]_[current_kinkphones_color]_[kinky_headphones_on? "on" : "off"]"

/obj/item/clothing/ears/earmuffs/kinky_headphones/proc/toggle(owner)
...
/datum/action/item_action/toggle_kinky_headphones/action_activate()
	var/obj/item/clothing/ears/earmuffs/kinky_headphones/headphones = target
