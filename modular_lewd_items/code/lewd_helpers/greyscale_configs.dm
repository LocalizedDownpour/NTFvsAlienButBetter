/*
*	NSFW GREYSCALE CONFIGS TO GO IN HERE
*/

// MASKS

/datum/greyscale_config/ballgag
	name = "Ball Gag"
	icon_file = 'modular_lewd_items/icons/obj/lewd_clothing/lewd_masks.dmi'
	json_config = 'modular_lewd_items/json_configs/mask/ballgag.json'

/datum/greyscale_config/ballgag/worn
	name = "Ball Gag (Worn)"
	icon_file = 'modular_lewd_items/icons/mob/lewd_clothing/lewd_masks.dmi'
	json_config = 'modular_lewd_items/json_configs/mask/ballgag_worn.json'

/datum/greyscale_config/ballgag/left_hand
	name = "Ball Gag (Inhand, Left)"
	icon_file = 'modular_lewd_items/icons/mob/lewd_inhands/lewd_inhand_left.dmi'
	json_config = 'modular_lewd_items/json_configs/mask/ballgag_inhand_left.json'

/datum/greyscale_config/ballgag/right_hand
	name = "Ball Gag (Inhand, Right)"
	icon_file = 'modular_lewd_items/icons/mob/lewd_inhands/lewd_inhand_right.dmi'
	json_config = 'modular_lewd_items/json_configs/mask/ballgag_inhand_right.json'

/datum/greyscale_config/ballgag/choking_small
	name = "Choke Gag (Small)"
	json_config = 'modular_lewd_items/json_configs/mask/choking_small.json'

/datum/greyscale_config/ballgag/choking_medium
	name = "Choke Gag (Medium)"
	json_config = 'modular_lewd_items/json_configs/mask/choking_medium.json'

/datum/greyscale_config/ballgag/choking_large
	name = "Choke Gag (Large)"
	json_config = 'modular_lewd_items/json_configs/mask/choking_large.json'

//shibari stand

/datum/greyscale_config/shibari_stand
	name = "Shibari Stand"
	icon_file = 'modular_lewd_items/icons/obj/lewd_structures/shibari_stand.dmi'
	json_config = 'modular_lewd_items/json_configs/structures/shibari_stand.json'

/datum/greyscale_config/shibari_stand_item
	name = "Shibari Stand (Item)"
	icon_file = 'modular_lewd_items/icons/obj/lewd_structures/bdsm_furniture.dmi'
	json_config = 'modular_lewd_items/json_configs/items/shibari_stand_item.json'

/datum/greyscale_config/shibari_stand_ropes
	name = "Shibari Stand (Rope Overlays)"
	icon_file = 'modular_lewd_items/icons/obj/lewd_structures/shibari_stand.dmi'
	json_config = 'modular_lewd_items/json_configs/structures/shibari_stand_ropes.json'

//shibari

/datum/greyscale_config/shibari_rope
	name = "Shibari Rope"
	icon_file = 'modular_lewd_items/icons/obj/lewd_items/lewd_items.dmi'
	json_config = 'modular_lewd_items/json_configs/items/shibari.json'

/datum/greyscale_config/shibari_rope_inhand_left
	name = "Shibari Rope (Inhand, Left)"
	icon_file = 'modular_lewd_items/icons/mob/lewd_inhands/lewd_inhand_left.dmi'
	json_config = 'modular_lewd_items/json_configs/items/shibari_inhands.json'

/datum/greyscale_config/shibari_rope_inhand_right
	name = "Shibari Rope (Inhand, Right)"
	icon_file = 'modular_lewd_items/icons/mob/lewd_inhands/lewd_inhand_right.dmi'
	json_config = 'modular_lewd_items/json_configs/items/shibari_inhands.json'

/datum/greyscale_config/shibari_rope/med
	json_config = 'modular_lewd_items/json_configs/items/shibari_med.json'

/datum/greyscale_config/shibari_rope/high
	json_config = 'modular_lewd_items/json_configs/items/shibari_high.json'

/datum/greyscale_config/shibari
	name = "Shibari"
	icon_file = 'modular_lewd_items/icons/obj/lewd_clothing/lewd_uniform.dmi'
	json_config = 'modular_lewd_items/json_configs/clothes/body.json'

/datum/greyscale_config/shibari/worn
	name = "Shibari (Worn)"
	icon_file = 'modular_lewd_items/icons/mob/lewd_clothing/lewd_uniform/lewd_uniform.dmi'
	json_config = 'modular_lewd_items/json_configs/clothes/body.json'

/datum/greyscale_config/shibari/worn/digi
	name = "Shibari (Worn, Digi)"
	icon_file = 'modular_lewd_items/icons/mob/lewd_clothing/lewd_uniform/lewd_uniform-digi.dmi'
	json_config = 'modular_lewd_items/json_configs/clothes/body.json'

/datum/greyscale_config/shibari/worn/taur_snake
	name = "Shibari (Worn, Taur Snake)"
	icon_file = 'modular_lewd_items/icons/mob/lewd_clothing/lewd_uniform/lewd_uniform-snake.dmi'
	json_config = 'modular_lewd_items/json_configs/clothes/body.json'

/datum/greyscale_config/shibari/worn/taur_paw
	name = "Shibari (Worn, Taur Paw)"
	icon_file = 'modular_lewd_items/icons/mob/lewd_clothing/lewd_uniform/lewd_uniform-paw.dmi'
	json_config = 'modular_lewd_items/json_configs/clothes/body.json'

/datum/greyscale_config/shibari/worn/taur_hoof
	name = "Shibari (Worn, Taur Hoof)"
	icon_file = 'modular_lewd_items/icons/mob/lewd_clothing/lewd_uniform/lewd_uniform-hoof.dmi'
	json_config = 'modular_lewd_items/json_configs/clothes/body.json'

/datum/greyscale_config/shibari/worn/teshari
	name = "Shibari (Worn, Teshari)"
	icon_file = 'modular_lewd_items/icons/mob/lewd_clothing/lewd_uniform/lewd_uniform_teshari.dmi'
	json_config = 'modular_lewd_items/json_configs/clothes/body.json'

//body

/datum/greyscale_config/shibari/body
	name = "Shibari Body"
	json_config = 'modular_lewd_items/json_configs/clothes/body.json'

/datum/greyscale_config/shibari/body/worn
	name = "Shibari Body (Worn)"
	icon_file = 'modular_lewd_items/icons/mob/lewd_clothing/lewd_uniform/lewd_uniform.dmi'
	json_config = 'modular_lewd_items/json_configs/clothes/body.json'

/datum/greyscale_config/shibari/body/worn/digi
	name = "Shibari Body (Worn, Digi)"
	icon_file = 'modular_lewd_items/icons/mob/lewd_clothing/lewd_uniform/lewd_uniform-digi.dmi'
	json_config = 'modular_lewd_items/json_configs/clothes/body.json'

/datum/greyscale_config/shibari/body/worn/taur_snake
	name = "Shibari Body (Worn, Taur Snake)"
	icon_file = 'modular_lewd_items/icons/mob/lewd_clothing/lewd_uniform/lewd_uniform-snake.dmi'
	json_config = 'modular_lewd_items/json_configs/clothes/body.json'

/datum/greyscale_config/shibari/body/worn/taur_paw
	name = "Shibari Body (Worn, Taur Paw)"
	icon_file = 'modular_lewd_items/icons/mob/lewd_clothing/lewd_uniform/lewd_uniform-paw.dmi'
	json_config = 'modular_lewd_items/json_configs/clothes/body.json'

/datum/greyscale_config/shibari/body/worn/taur_hoof
	name = "Shibari Body (Worn, Taur Hoof)"
	icon_file = 'modular_lewd_items/icons/mob/lewd_clothing/lewd_uniform/lewd_uniform-hoof.dmi'
	json_config = 'modular_lewd_items/json_configs/clothes/body.json'

/datum/greyscale_config/shibari/body/worn/teshari
	name = "Shibari Body (Worn, Teshari)"
	icon_file = 'modular_lewd_items/icons/mob/lewd_clothing/lewd_uniform/lewd_uniform_teshari.dmi'
	json_config = 'modular_lewd_items/json_configs/clothes/body.json'

//groin

/datum/greyscale_config/shibari/groin
	name = "Shibari Groin"
	json_config = 'modular_lewd_items/json_configs/clothes/groin.json'

/datum/greyscale_config/shibari/groin/worn
	name = "Shibari Groin (Worn)"
	icon_file = 'modular_lewd_items/icons/mob/lewd_clothing/lewd_uniform/lewd_uniform.dmi'
	json_config = 'modular_lewd_items/json_configs/clothes/groin.json'

/datum/greyscale_config/shibari/groin/worn/digi
	name = "Shibari Groin (Worn, Digi)"
	icon_file = 'modular_lewd_items/icons/mob/lewd_clothing/lewd_uniform/lewd_uniform-digi.dmi'
	json_config = 'modular_lewd_items/json_configs/clothes/groin.json'

/datum/greyscale_config/shibari/groin/worn/taur_snake
	name = "Shibari Groin (Worn, Taur Snake)"
	icon_file = 'modular_lewd_items/icons/mob/lewd_clothing/lewd_uniform/lewd_uniform-snake.dmi'
	json_config = 'modular_lewd_items/json_configs/clothes/groin.json'

/datum/greyscale_config/shibari/groin/worn/taur_paw
	name = "Shibari Groin (Worn, Taur Paw)"
	icon_file = 'modular_lewd_items/icons/mob/lewd_clothing/lewd_uniform/lewd_uniform-paw.dmi'
	json_config = 'modular_lewd_items/json_configs/clothes/groin.json'

/datum/greyscale_config/shibari/groin/worn/taur_hoof
	name = "Shibari Groin (Worn, Taur Hoof)"
	icon_file = 'modular_lewd_items/icons/mob/lewd_clothing/lewd_uniform/lewd_uniform-hoof.dmi'
	json_config = 'modular_lewd_items/json_configs/clothes/groin.json'

/datum/greyscale_config/shibari/groin/worn/teshari
	name = "Shibari Groin (Worn, Teshari)"
	icon_file = 'modular_lewd_items/icons/mob/lewd_clothing/lewd_uniform/lewd_uniform_teshari.dmi'
	json_config = 'modular_lewd_items/json_configs/clothes/groin.json'

//fullbody

/datum/greyscale_config/shibari/fullbody
	name = "Shibari Fullbody"
	json_config = 'modular_lewd_items/json_configs/clothes/fullbody.json'

/datum/greyscale_config/shibari/worn/fullbody
	name = "Shibari Fullbody (Worn)"
	icon_file = 'modular_lewd_items/icons/mob/lewd_clothing/lewd_uniform/lewd_uniform.dmi'
	json_config = 'modular_lewd_items/json_configs/clothes/fullbody.json'

/datum/greyscale_config/shibari/fullbody/worn/digi
	name = "Shibari Fullbody (Worn, Digi)"
	icon_file = 'modular_lewd_items/icons/mob/lewd_clothing/lewd_uniform/lewd_uniform-digi.dmi'
	json_config = 'modular_lewd_items/json_configs/clothes/fullbody.json'

/datum/greyscale_config/shibari/fullbody/worn/taur_snake
	name = "Shibari Fullbody (Worn, Taur Snake)"
	icon_file = 'modular_lewd_items/icons/mob/lewd_clothing/lewd_uniform/lewd_uniform-snake.dmi'
	json_config = 'modular_lewd_items/json_configs/clothes/fullbody.json'

/datum/greyscale_config/shibari/fullbody/worn/taur_paw
	name = "Shibari Fullbody (Worn, Taur Paw)"
	icon_file = 'modular_lewd_items/icons/mob/lewd_clothing/lewd_uniform/lewd_uniform-paw.dmi'
	json_config = 'modular_lewd_items/json_configs/clothes/fullbody.json'

/datum/greyscale_config/shibari/fullbody/worn/taur_hoof
	name = "Shibari Fullbody (Worn, Taur Hoof)"
	icon_file = 'modular_lewd_items/icons/mob/lewd_clothing/lewd_uniform/lewd_uniform-hoof.dmi'
	json_config = 'modular_lewd_items/json_configs/clothes/fullbody.json'

/datum/greyscale_config/shibari/fullbody/worn/teshari
	name = "Shibari Fullbody (Worn, Teshari)"
	icon_file = 'modular_lewd_items/icons/mob/lewd_clothing/lewd_uniform/lewd_uniform_teshari.dmi'
	json_config = 'modular_lewd_items/json_configs/clothes/fullbody.json'

//hands

/datum/greyscale_config/shibari/hands
	name = "Shibari Hands"
	icon_file = 'modular_lewd_items/icons/obj/lewd_clothing/lewd_gloves.dmi'
	json_config = 'modular_lewd_items/json_configs/clothes/hands.json'

/datum/greyscale_config/shibari/hands/worn
	name = "Shibari Hands (Worn)"
	icon_file = 'modular_lewd_items/icons/mob/lewd_clothing/lewd_gloves.dmi'
	json_config = 'modular_lewd_items/json_configs/clothes/hands.json'

/datum/greyscale_config/shibari/hands/worn/teshari
	name = "Shibari Hands (Worn, Teshari)"
	icon_file = 'modular_lewd_items/icons/mob/lewd_clothing/lewd_gloves_teshari.dmi'
	json_config = 'modular_lewd_items/json_configs/clothes/hands.json'

//legs

/datum/greyscale_config/shibari/legs
	name = "Shibari Legs"
	icon_file = 'modular_lewd_items/icons/obj/lewd_clothing/lewd_shoes.dmi'
	json_config = 'modular_lewd_items/json_configs/clothes/legs.json'

/datum/greyscale_config/shibari/legs/worn
	name = "Shibari Legs (Worn)"
	icon_file = 'modular_lewd_items/icons/mob/lewd_clothing/lewd_shoes.dmi'
	json_config = 'modular_lewd_items/json_configs/clothes/legs.json'

/datum/greyscale_config/shibari/legs/worn/digi
	name = "Shibari Legs Digi (Worn, Digi)"
	icon_file = 'modular_lewd_items/icons/mob/lewd_clothing/lewd_shoes_digi.dmi'
	json_config = 'modular_lewd_items/json_configs/clothes/legs.json'

/datum/greyscale_config/shibari/legs/worn/teshari
	name = "Shibari Legs (Worn, Teshari)"
	icon_file = 'modular_lewd_items/icons/mob/lewd_clothing/lewd_shoes_teshari.dmi'
	json_config = 'modular_lewd_items/json_configs/clothes/legs.json'

