...
/obj/item/clothing/sextoy/magic_wand/Initialize(mapload)
	...
	magicwand_overlay = mutable_appearance('modular_lewd_items/icons/mob/lewd_items/lewd_items.dmi', "magicwand", -BODY_FRONT_UNDER_CLOTHES) //two arguments
...
/obj/item/clothing/sextoy/magic_wand/lewd_equipped(mob/living/carbon/user, slot)
	. = ..()
	if(vibration_mode == MAGIC_WAND_MODE_OFF || !iscarbon(user))
		return
	if(src == user.lewd_penis || src == user.lewd_vagina)
...
/obj/item/clothing/sextoy/magic_wand/process(seconds_per_tick)
	var/mob/living/carbon/current_user = loc
	if(!iscarbon(current_user) || current_user.stat == DEAD)
		return FALSE
	...
	switch(user.zone_selected)
		if(BODY_ZONE_PRECISE_GROIN)
			if(carbon_target)
				...
				var/organ_to_use = carbon_target.sexcon_has_penis() ? "penis" : "vagina"
		...
		if(BODY_ZONE_CHEST)
			if(carbon_target)
				...
				message = (user == target) ? pick("massages their breasts with the [src]",
						"[first_adjective] teases their tits with [src]") \
					: pick("[second_adjective] teases [target]'s breasts with [src]",
						"uses [src] to [vibration_mode == MAGIC_WAND_MODE_LOW ? "slowly" : ""] massage [target]'s tits",
						"uses [src] to tease [target]'s boobs")
