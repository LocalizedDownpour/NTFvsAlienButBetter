...
/obj/item/clothing/mask/leatherwhip
	...
	hitsound = 'modular_lewd_items/sounds/slap.ogg'
	...
	switch(user.zone_selected) //to let code know what part of body we gonna whip
		if(BODY_ZONE_L_LEG, BODY_ZONE_R_LEG)
			if(current_whip_type == "hard")
				...
				conditional_pref_sound(loc, 'modular_lewd_items/sounds/slap.ogg', 100)
			else
				message = (user == target) ? pick("knocks [target.p_them()]self down with [src]",
				...
				conditional_pref_sound(loc, 'modular_lewd_items/sounds/slap.ogg', 60)
		...
		if(BODY_ZONE_HEAD)
			...
			target.adjust_pain(5)
			conditional_pref_sound(loc, 'modular_lewd_items/sounds/latex.ogg', 80)
