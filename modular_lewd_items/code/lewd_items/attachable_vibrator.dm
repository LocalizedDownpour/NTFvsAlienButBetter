...
/obj/item/clothing/sextoy/eggvib/lewd_equipped(mob/living/carbon/user, slot, initial)
	. = ..()
	if(!iscarbon(user))
		return
...
/obj/item/clothing/sextoy/eggvib/attack(mob/living/target, mob/living/user)
	...
	if(user == carbon_target)
		carbon_target.visible_message(span_purple("[user] inserts [src] into [carbon_target.p_their()] [part_name]."), span_purple("You insert [src] into your [part_name]."))
	else
		carbon_target.visible_message(span_purple("[user] inserts [src] into [carbon_target]'s [part_name]."), span_purple("[user] inserts [src] into your [part_name]."))
	conditional_pref_sound(loc, 'modular_lewd_items/sounds/squelch2.ogg', 30, TRUE)
...
/obj/item/clothing/sextoy/eggvib/signalvib/receive_signal(datum/signal/signal)
	...
	var/mob/living/carbon/vibrated = loc
	if(!iscarbon(vibrated))
		return
	if(toy_on)
		if(src == vibrated.lewd_penis || src == vibrated.lewd_vagina || src == vibrated.lewd_anus)
			to_chat(vibrated, span_purple("You feel pleasant vibrations deep below..."))
		else if(src == vibrated.lewd_nipples)
