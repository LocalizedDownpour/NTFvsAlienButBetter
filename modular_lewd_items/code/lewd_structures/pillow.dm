...
//and there is code for successful check, so we are hitting someone with a pillow
	...
	switch(user.zone_selected) //to let code know what part of body we gonna hit
		...
		if(BODY_ZONE_HEAD)
			...
			playsound(loc, 'modular_lewd_items/sounds/hug.ogg', 50, 1, -1)
...
/obj/structure/bed/pillow_tiny
	...
	icon = 'modular_lewd_items/icons/obj/lewd_structures/pillows.dmi'
	icon_state = "pillow_pink_round"
	base_icon_state = "pillow"
	var/current_color = "pink"
	var/current_form = "round"
...
/obj/structure/bed/pillow_tiny/attackby(obj/item/used_item, mob/living/user, params)
	if(istype(used_item, /obj/item/fancy_pillow))
		...
		var/obj/structure/bed/chair/pillow_small/pillow_pile
...
/obj/structure/bed/chair/pillow_small
...
/obj/structure/bed/chair/pillow_small/Initialize(mapload)
	update_icon()
	// elevation handled
	return ..()

/obj/structure/bed/chair/pillow_small/post_buckle_mob(mob/living/affected_mob)
	. = ..()
	density = TRUE
	//Push them up from the normal lying position
	affected_mob.pixel_y = affected_mob.base_pixel_y + 2

/obj/structure/bed/chair/pillow_small/post_unbuckle_mob(mob/living/affected_mob)
	. = ..()
	density = FALSE
	//Set them back down to the normal lying position
	affected_mob.pixel_y = affected_mob.base_pixel_y

/obj/structure/bed/chair/pillow_small/update_icon_state()
	. = ..()
	icon_state = "[base_icon_state]_[current_color]"

//Removing pillow from a pile
/obj/structure/bed/chair/pillow_small/click_alt(mob/user)
...
/obj/structure/bed/chair/pillow_small/attackby(obj/item/used_item, mob/living/user, params)
	if(istype(used_item, /obj/item/fancy_pillow))
		var/obj/item/fancy_pillow/used_pillow = used_item
...
/obj/structure/bed/chair/pillow_small/mouse_drop_dragged(atom/over, mob/user, src_location, over_location, params)
	return
...
/obj/structure/bed/pillow_large/click_alt(mob/user)
	to_chat(user, span_notice("You take [src] from the pile."))
	var/obj/item/fancy_pillow/taken_pillow = new()
	var/obj/structure/bed/chair/pillow_small/pillow_pile = new(get_turf(src))
	user.put_in_hands(taken_pillow)
	//magic
	pillow_pile.current_color = current_color
